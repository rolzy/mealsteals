...
async def get_all_restaurants(
    limit: int = Query(default=100, ge=1, le=1000, description="Maximum number of restaurants to return"),
    suburb: Optional[str] = Query(default=None, description="Filter by suburb (case-insensitive)"),
    postcode: Optional[str] = Query(default=None, description="Filter by postcode"),
    is_open_now: Optional[bool] = Query(default=None, description="Filter by whether restaurant is currently open")
):
    """Get all restaurants with optional filters"""
    logger.info(f"Getting restaurants - limit: {limit}, suburb: {suburb}, postcode: {postcode}, is_open_now: {is_open_now}")
    ...
    try:
        ...
        raise InternalServerErrorException(detail=f"Failed to retrieve restaurants: {str(e)}")
...
async def get_restaurant(
    ...
    try:
        ...
        if not restaurant:
            ...
            raise NotFoundException(detail=f"Restaurant with ID {restaurant_id} not found")
        ...
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise InternalServerErrorException(detail=f"Failed to retrieve restaurant: {str(e)}")
...
async def create_restaurant(restaurant_data: RestaurantCreate):
    ...
    try:
        ...
        existing_restaurant = restaurant_service.get_restaurant_by_gmaps_id(restaurant_data.gmaps_id)
        if existing_restaurant:
            logger.warning(f"Restaurant with gmaps_id {restaurant_data.gmaps_id} already exists")
        ...
        logger.info(f"Successfully created restaurant: {restaurant.name} (UUID: {restaurant.uuid})")
        return restaurant
        ...
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise InternalServerErrorException(detail=f"Failed to create restaurant: {str(e)}")
...
async def update_restaurant(
    ...
    try:
        ...
        if restaurant_data.gmaps_id != existing_restaurant.gmaps_id:
            existing_gmaps_restaurant = restaurant_service.get_restaurant_by_gmaps_id(restaurant_data.gmaps_id)
            if existing_gmaps_restaurant and str(existing_gmaps_restaurant.uuid) != restaurant_id:
                logger.warning(f"Cannot update: gmaps_id {restaurant_data.gmaps_id} already exists for another restaurant")
                raise BadRequestException(
        ...
        if not updated_restaurant:
            logger.error(f"Update operation returned None for restaurant {restaurant_id}")
        ...
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise InternalServerErrorException(detail=f"Failed to update restaurant: {str(e)}")
...
async def delete_restaurant(
    ...
    try:
        ...
        if not success:
            logger.error(f"Delete operation returned False for restaurant {restaurant_id}")
        ...
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise InternalServerErrorException(detail=f"Failed to delete restaurant: {str(e)}")


@router.post("/restaurants/search", response_model=RestaurantSearchResultResponse)
async def search_restaurants(
    ...
    logger.info(f"Restaurant search request: address='{search_request.address}', radius={search_request.radius}m, filters: suburb={suburb}, postcode={postcode}, is_open_now={is_open_now}")
    ...
    try:
        ...
        filtered_restaurants, restaurants_created, restaurants_updated = restaurant_service.search_and_filter_restaurants(
        ...
        logger.info(f"Search completed: {restaurants_created} created, {restaurants_updated} updated, {len(filtered_restaurants)} returned")
        return response
